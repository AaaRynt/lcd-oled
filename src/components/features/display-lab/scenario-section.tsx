...
export function ScenarioSection() {
  ...
  return (
    <section id="scenarios" className="mx-auto max-w-300 px-4 py-20 sm:px-6 lg:px-8">
      <SectionHeading title="不同场景下，优势会重新排序" description="选择维度后，可以看到 LCD 与 OLED 在该使用方式中的相对取舍。这里不使用虚假的精确分数，只呈现常见体验倾向。" />

      <div className="mt-10 rounded-3xl border border-white/10 bg-white/6 p-4 sm:p-6">
        <div role="tablist" aria-label="使用场景维度" className="flex flex-wrap gap-2">
          {dimensionOptions.map((option, index) => {
            ...
            return (
              <button key={option.id} aria-controls={`dimension-panel-${option.id}`} aria-selected={isActive} className={`rounded-full px-4 py-2 text-sm font-medium transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-cyan-300 ${isActive ? 'bg-zinc-50 text-zinc-950' : 'border border-white/10 bg-zinc-950/60 text-zinc-300 hover:border-white/20 hover:bg-white/8 hover:text-zinc-50'}`} id={`dimension-tab-${option.id}`} onClick={() => setActiveId(option.id)} onKeyDown={(event) => handleTabKeyDown(event, index)} role="tab" tabIndex={isActive ? 0 : -1} type="button">
                {option.label}
              </button>
            )
          })}
      ...
      <div className="mt-20">
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-2">
          {scenarioCards.map((card) => {
            ...
            return (
              <article key={card.title} className="rounded-3xl border border-white/10 bg-zinc-950/70 p-6 transition hover:border-white/18 hover:bg-white/4.5">
